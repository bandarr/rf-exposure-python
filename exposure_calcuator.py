...
class ExposureCalculator:
    ...
    def calculate_uncontrolled_safe_distance( self, freq_values: FrequencyValues, cable_values: CableValues, transmitter_power: int, feedline_length: int, duty_cycle: float, uncontrolled_percentage_30_minutes: float ) -> float:
        gamma = self.__calculate_reflection_coefficient( freq_values )

        feedline_loss_per_100ft_at_frequency = self.__calculate_feedline_loss_per_100ft_at_frequency( freq_values, cable_values )

        feedline_loss_for_matched_load_at_frequency = self.__calculate_feedline_loss_for_matched_load_at_frequency( feedline_length, feedline_loss_per_100ft_at_frequency )

        feedline_loss_for_matched_load_at_frequency_percentage = self.__calculate_feedline_loss_for_matched_load_at_frequency_percentage( feedline_loss_for_matched_load_at_frequency)

        gamma_squared = abs( gamma )**2

        feedline_loss_for_swr = self.__calculate_feedline_loss_for_swr( feedline_loss_for_matched_load_at_frequency_percentage, gamma_squared )

        feedline_loss_for_swr_percentage = self.__calculate_feedline_loss_for_swr_percentage( feedline_loss_for_swr )
